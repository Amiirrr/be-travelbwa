...
import { chai, expect, should, assert } from '@bundled-es-modules/chai';
import fs from 'fs';
...
describe('API ENDPOINT TESTING', () => {
    ...
    it('GET Detail Page', (done) => {
        chai.request(server).get('/api/v1/member/detail-page/5e96cbe292b97300fc902223').end((err, res) => {
            expect(err).to.be.null
            expect(res).to.have.status(200)
            expect(res.body).to.be.an('object')
    ...
    it('POST Booking Page', (done) => {
        const image = __dirname + '/src/test/buktibayar.jpeg';
        const dataSample = {
            image,
        ...
        chai.request(server).post('/api/v1/member/booking-page')
